...
# Lista estándar de variables de suelo (por si la quieres reutilizar)
SOIL_VARIABLES = [
...
def load_data():
    ...
    if not icd_registro_path.exists():
        raise FileNotFoundError(
            ...
            "Asegúrate de haber ejecutado el notebook 06_ICD_por_variable."
        )
    ...
    df[icd_total_cols] = df[icd_total_cols].apply(
    ...
    if forecast_path.exists():
        ...
        # asegurar tipo de fecha si existe columna 'fecha'
        if "fecha" in df_forecast.columns:
            df_forecast["fecha"] = pd.to_datetime(
                df_forecast["fecha"], errors="coerce"
            )
    else:
        df_forecast = None
...
def get_group_col(df: pd.DataFrame) -> str:
    ...
    'dep_norm' si existe, en caso contrario 'departamento'.
    """
